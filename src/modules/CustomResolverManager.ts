...
export class CustomResolverManager {
  ...
  private static zoteroCustomResolversPrefKey =
    "extensions.zotero.findPDFs.resolvers" as const;
  private static customResolversPrefKey = "resolvers" as const;
  private static customResolversLargerPrefKey =
    "userCustomResolvers" as const;
  ...
  appendCustomResolversInZotero(
    ...
    this.customResolversInZotero = this.customResolversInZotero.concat(
      resolvers.filter(
        (value) =>
          this.customResolversInZotero.findIndex((e) => isCustomResolverEqual(e, value)) <
          0,
      ),
    );
    this.customResolvers = this.customResolvers.concat(
      resolvers.filter(
        (value) => this.customResolvers.findIndex((e) => isCustomResolverEqual(e, value)) < 0,
      ),
    );
  }
